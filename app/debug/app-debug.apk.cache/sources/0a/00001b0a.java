...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m1548plusAssignLRDsOJo(long duration) {
        long newReading;
        long longDelta = Duration.m1460toLongimpl(duration, getUnit());
        if (longDelta != Long.MIN_VALUE && longDelta != Long.MAX_VALUE) {
            long j = this.reading;
            newReading = j + longDelta;
            if ((j ^ longDelta) >= 0 && (j ^ newReading) < 0) {
                m1547overflowLRDsOJo(duration);
            }
        } else {
            double delta = Duration.m1457toDoubleimpl(duration, getUnit());
    ...
    private final void m1547overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + "ns is advanced by " + ((Object) Duration.m1463toStringimpl(duration)) + '.');
    }
}