...
public final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cancellableContinuation) {
            ...
            while (true) {
                if (cur$iv$iv.getId() < id$iv || cur$iv$iv.getRemoved()) {
                    ...
                    if (it$iv$iv$iv == ConcurrentLinkedListKt.CLOSED) {
                        m1639constructorimpl = SegmentOrClosed.m1639constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    m1639constructorimpl = SegmentOrClosed.m1639constructorimpl(cur$iv$iv);
                    curTail = s$iv2;
                    break;
                }
            }
            s$iv = m1639constructorimpl;
            if (SegmentOrClosed.m1644isClosedimpl(s$iv)) {
                break;
            }
            Segment to$iv$iv = SegmentOrClosed.m1642getSegmentimpl(s$iv);
            while (true) {
        ...
        SemaphoreSegment segment = (SemaphoreSegment) SegmentOrClosed.m1642getSegmentimpl(s$iv);
        i2 = SemaphoreKt.SEGMENT_SIZE;
        int i3 = (int) (enqIdx % i2);
        if (!segment.acquirers.compareAndSet(i3, null, cancellableContinuation)) {
            symbol = SemaphoreKt.PERMIT;
            symbol2 = SemaphoreKt.TAKEN;
            if (segment.acquirers.compareAndSet(i3, symbol, symbol2)) {
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        symbol = SemaphoreKt.PERMIT;
        Object value$iv = segment.acquirers.getAndSet(i4, symbol);
        if (value$iv == null) {
            ...
            return !segment.acquirers.compareAndSet(i4, symbol3, symbol4);
        }
