...
public final class LongSaturatedMathKt {
    ...
    public static final long m1542saturatingAddpTJri5U(long longNs, long duration) {
        long durationNs = Duration.m1434getInWholeNanosecondsimpl(duration);
        if (((longNs - 1) | 1) != Long.MAX_VALUE) {
            if ((1 | (durationNs - 1)) == Long.MAX_VALUE) {
                return m1543saturatingAddInHalvespTJri5U(longNs, duration);
            }
        ...
        return m1541checkInfiniteSumDefinedPjuGub4(longNs, duration, durationNs);
    }

    /* renamed from: checkInfiniteSumDefined-PjuGub4  reason: not valid java name */
    private static final long m1541checkInfiniteSumDefinedPjuGub4(long longNs, long duration, long durationNs) {
        if (!Duration.m1446isInfiniteimpl(duration) || (longNs ^ durationNs) >= 0) {
            return longNs;
        }
    ...
    private static final long m1543saturatingAddInHalvespTJri5U(long longNs, long duration) {
        long half = Duration.m1417divUwyO8pc(duration, 2);
        if (((Duration.m1434getInWholeNanosecondsimpl(half) - 1) | 1) == Long.MAX_VALUE) {
            return (long) (longNs + Duration.m1457toDoubleimpl(duration, DurationUnit.NANOSECONDS));
        }
        return m1542saturatingAddpTJri5U(m1542saturatingAddpTJri5U(longNs, half), half);
    }

    public static final long saturatingDiff(long valueNs, long originNs) {
        if ((1 | (originNs - 1)) == Long.MAX_VALUE) {
            return Duration.m1466unaryMinusUwyO8pc(DurationKt.toDuration(originNs, DurationUnit.DAYS));
        }
        ...
        if (((result ^ valueNs) & (~(result ^ originNs))) < 0) {
            ...
            return Duration.m1450plusLRDsOJo(duration, DurationKt.toDuration(resultNs, DurationUnit.NANOSECONDS));
        }
